...
// Types for daily reconciliation report
...
reportsRouter.get('/reconciliation', async (c) => {
  const db = c.env.AURA_DB;
  ...
  const digitalTotal = paymentMethodSummary
    .filter(p => p.method !== 'cash')
    .reduce((sum, p) => sum + p.total, 0);

  // Build category aggregation
  const catMap = new Map<string, { items_sold: number; revenue: number }>();
  for (const row of ordersWithItems || []) {
    try {
      const items = JSON.parse(row.items || '[]');
      if (!Array.isArray(items)) continue;
      for (const item of items) {
        const cat = item.category || item.category_name || 'Uncategorized';
        const qty = item.qty || item.quantity || 1;
        const price = item.price || item.unit_price || 0;
        const existing = catMap.get(cat) || { items_sold: 0, revenue: 0 };
        existing.items_sold += qty;
        existing.revenue += price * qty;
