...
// ── POST /api/orders ─────────────────────────────────────────────────────
// Body: { customer_name, phone?, table_id?, notes?, items: [{product_id, quantity, price}] }
ordersRouter.post('/', async (c) => {
  ...
  return c.json({
    success: true,
    data: { id: orderId, status: 'Bep tiep nhan', total_amount },
  }, 201);
});
...
ordersRouter.patch('/:id/status', async (c) => {
  ...
  // Update KV flag so KDS pollers see status change too
  await kv.put(KV_LATEST_KEY, now);
