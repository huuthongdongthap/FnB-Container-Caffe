 ...
 * MRR Container Lease Subscription System.
 *
 * Model: Rent container units to F&B vendors on monthly recurring contracts.
 *
 * Endpoints:
 *   GET    /plans              — List all active lease plans
 *   GET    /plans/:id          — Get single plan details
 *   POST   /plans              — Create plan (admin)
 *   PUT    /plans/:id          — Update plan (admin)
 *
 *   GET    /                   — List all subscriptions
 *   GET    /stats              — MRR + churn KPIs
 *   GET    /mrr                — MRR trend (daily snapshots)
 ...
 *   POST   /invoices/generate  — Generate monthly invoices (cron)
 */
...
function generateId(prefix: string, len = 8): string {
...
async function updateMRRSnapshot(db: any): Promise<void> {
  const date = today();
  ...
  ).bind(monthStart.toISOString()) as any).first();
...
subscriptionsRouter.get('/stats', async (c) => {
  ...
  `).first<{ under_1m: number; from_1m_to_3m: number; from_3m_to_5m: number; above_5m: number }>();
