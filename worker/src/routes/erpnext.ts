...
import { erpnextConfigureSchema, erpnextLeadSchema, erpnextTagSchema, erpnextUpdateCustomerSchema } from '../lib/validators';
...
export async function handleErpnextRequest(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (method === 'PUT' && path.startsWith('/customer/')) {
      const customerId = path.replace('/customer/', '');
      const putParsed = erpnextUpdateCustomerSchema.safeParse(await request.json());
