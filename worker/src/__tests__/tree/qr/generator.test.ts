...
describe('generateSlug', () => {
  ...
  it('VIP01 private → vip01-private', () => {
...
describe('bulkGenerateSlugs', () => {
  ...
  it('inserts slugs for tables missing them', async () => {
    ...
    const result = await bulkGenerateSlugs(db, tables);
  ...
  it('preserves pre-existing slug, generates new for missing table', async () => {
    ...
    const result = await bulkGenerateSlugs(db, tables);
    expect(result.get(1)).toBe(existingSlug); // preserved
    expect(result.get(2)).toBe('t02-outdoor');  // generated fresh
  });
});
