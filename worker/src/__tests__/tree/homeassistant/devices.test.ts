/**
 * Unit tests for HA device tree functions
 */
...
const origAll = db.prepare = vi.fn(() => {
...
if (rows.length > 0 && stmt.all) stmt.all.mockResolvedValue({ results: rows, success: true });
if (rows.length > 0 && stmt.first) stmt.first.mockResolvedValue(rows[0]);
...
// getDeviceState returns Response — check status
...
const prepareMock = vi.fn().mockReturnValue({ bind: vi.fn().mockReturnThis(), run: runMock, first: firstMock });
(db as Record<string, unknown>).prepare = prepareMock;
...
const prepareMock = vi.fn().mockReturnValue({ bind: vi.fn().mockReturnThis(), run: vi.fn(), first: firstMock });
(db as Record<string, unknown>).prepare = prepareMock;
...
await expect(cacheDeviceState(env, {
entity_id: 'light.new_device',
state: 'on',
attributes: { friendly_name: 'New Device' },
last_changed: new Date().toISOString(),
last_updated: new Date().toISOString(),
} as Parameters<typeof cacheDeviceState>[2])).resolves.toBeUndefined();
expect(prepareMock).toHaveBeenCalled();
});
});
