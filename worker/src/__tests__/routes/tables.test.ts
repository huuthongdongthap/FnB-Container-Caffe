  ...
  return {
    prepare: (sql: string) => {
      const stmt: any = {
        ...
        first: async () => {
          if (sql.includes('table_qr_codes') && sql.includes('WHERE slug')) return qrRow?.row ?? null;
          if (sql.includes('cafe_tables') && sql.includes('WHERE id')) return tableRow?.row ?? null;
          return null;
        },
        all: async () => {
          if (sql.includes('cafe_tables')) return { results: tables, success: true };
          if (sql.includes('table_qr_codes')) return { results: qrRows, success: true };
...
function execTables(path: string, db: any, extra: any = {}, method = 'GET'): Promise<Response> {
  ...
  return tablesRouter.fetch(new Request(`https://test.aura${path}`, { method }), env as any, ctx as any);
}

function execQR(path: string, db: any, extra: any = {}): Promise<Response> {
  const env = { ...createMockEnv(), AURA_DB: db, ...extra };
  const ctx = createMockContext();
  return qrRouter.fetch(new Request(`https://test.aura${path}`), env as any, ctx as any);
}
