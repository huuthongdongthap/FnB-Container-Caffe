...
function makeDB(rows?: Record<string, unknown>[], firstRow?: Record<string, unknown> | null) {
  ...
  return {
    prepare: stmt as unknown as ReturnType<typeof createMockDB>['prepare']
  };
}
...
function stubDB(
  ...
  (db as unknown as Record<string, unknown>).prepare = () => stmt() as any;
  return db;
}
...
describe('createHARouter', () => {
  describe('GET /devices', () => {
    it('returns 200 with devices array', async() => {
      ...
      const body: any = JSON.parse(await res.text() as any);
  ...
  describe('GET /devices/:entityId', () => {
    it('returns 200 with device data', async() => {
      ...
      expect(res.status).toBe(200);
      const body: any = JSON.parse(await res.text() as any);
      expect((body as any).device?.entity_id ?? (body as any).entity_id).toBe('light.kitchen');
    });
  });
  ...
  describe('POST /devices/:entityId/toggle', () => {
    it('returns 200 for valid toggle-on body', async() => {
      ...
      expect(res.status).toBe(200);
      const body: any = JSON.parse(await res.text() as any);
      expect((body as any).result?.state ?? (body as any).state).toBe('on');
    });
