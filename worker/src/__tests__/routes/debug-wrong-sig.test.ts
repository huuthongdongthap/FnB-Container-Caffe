...
import { createMockEnv, createMockContext } from './src/__tests__/test-utils';
    ...
    prepare: (sql: string) => {
      ...
      const stmt: any = {
        ...
        first: async () => {
          ...
          if (sql.includes('table_qr_codes WHERE slug')) return qrRows.length > 0 ? qrRows[0] : null;
          if (sql.includes('cafe_tables WHERE id')) return tableRow;
          return null;
        },
        all: async () => {
...
async function fetchTables(path: string, envOverrides: any = {}): Promise<Response> {
  ...
  return tablesRouter.fetch(req, env, ctx);
}
