...
| **M4** | AURA Online | Phase 5 | Digital Menu / Online Order / Pickup / Delivery / Customer Account / CRM **sharing the same catalog, pricing, order domain as the store** (v4 §15 channel law) | online order end-to-end on same core, no channel-specific business logic | ⏳ IN PROGRESS (Option A — Customer Account done; Option B — Digital Menu: GET /menu, customer-safe view strips cost/sku/supplier, category aggregation, availability filter, 7 tests; Option C — Online Order: POST /orders, price trust server-side, channel pickup/delivery, 11 tests; Option D — Pickup/Delivery: GET /fulfillment/locations + GET /orders/:id/fulfillment, ETA from prep baseline + queue depth, customer-scoped access, KV-backed pickup point w/ static fallback, 11 tests) |
...
3. `packages/domain/crm` — Customer Events, Visit, CRM profile, lookup — **done 2026-09-15** (aggregateEvents read-model, GET /api/crm/customers/:id/events + :id/profile, 9 tests)
