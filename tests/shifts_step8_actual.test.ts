...
function createMockD1(seedData: Record<string, any[]> = {}) {
 ...
 const conditions = whereMatch.map((w: string) => { const m = w.match(/WHERE\s+(\w+)\s*=\s*\?/i) || w.match(/AND\s+(\w+)\s*=\s*\?/i); return m ? m[1] : null; }).filter(Boolean);
 return conditions.every((col: string, i: number) => String(r[col]) === String(this._bindValues[i]));
 }) || null;
 }
 return rows[0] || null;
 }),
 all: vi.fn(async function () { const table = getPrimaryTable(q); const rows = (table && tableData[table]) ? tableData[table] : []; return { results: [...rows] }; }),
 run: vi.fn(async function () { return { success: true, meta: { last_row_id: tableData.shifts?.length || 0 } } as any; }),
 };
 return stmt;
 }),
 };
...
describe('POST /clock-in', () => {
 ...
 const res = await shiftsRouter.request('/clock-in', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ staff_id: 's1', staff_name: 'Staff One' }) }, env);
