...
vi.mock('../worker/src/middleware/auth.js', () => ({ requireAuth: () => { return async (c: any, next: any) => { c.set('user', { id: 'tu', role: 'staff' }); await next(); }; }, }));

describe('debug shifts A', () => {
...
// Same mock but WITH return wrapper
vi.mock('../worker/src/middleware/auth.js', () => ({ requireAuth: () => async (c: any, next: any) => { c.set('user', { id: 'tu2', role: 'staff' }); await next(); }, }));

describe('debug shifts B', () => {
  beforeEach(() => { vi.clearAllMocks(); });
 test('import and use', async () => {
 try {
 const mod = await import('../worker/src/routes/shifts');
