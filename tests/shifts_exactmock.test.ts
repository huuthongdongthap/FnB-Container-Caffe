...
// EXACT mock from shifts.test.ts lines 3-11
vi.mock('../worker/src/middleware/auth.js', () => ({ requireAuth: () => { return async (c: any, next: any) => { c.set('user', { id: 'test-user', email: 'staff@test.com', name: 'Test Staff', role: 'staff' }); await next(); }; }, }));

describe('test exact mock', () => {
  beforeEach(() => { vi.clearAllMocks(); });
  test('exact mock alone', async () => {
 try {
 ...
 console.log('EXACT MOCK Status:', res.status, 'Body:', text);
