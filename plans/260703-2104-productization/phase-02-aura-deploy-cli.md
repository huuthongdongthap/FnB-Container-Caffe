# Phase 2: Setup CLI Tool "aura-deploy" (8-10h)

**Date:** 2026-07-03
**Status:** Planned
**Prerequisite:** Phase 1 (branding isolation) must be complete -- brand.json, env-var-driven CSS, configurable deploy template.

---

## 2.1 Scaffold aura-deploy CLI (1.5h)
...
**Risks:** Low. Straightforward scaffolding.

---

## 2.2 Implement `init` command -- Interactive Wizard (2h)
...
**Acceptance Criteria:**
- `aura-deploy init` launches interactive prompt sequence
- Prompts for:
  - Cafe name (required, Unicode-friendly, e.g. "Cafe Xinh")
  - Domain slug (auto-suggested from name, editable, alphanumeric-dash only)
  - Primary color (hex, default `#C9A962` -- AURA gold)
  - Secondary color (hex, default `#1a1a2e`)
  - Cafe tagline / description (optional, max 100 chars)
  - Admin email (required, email validated)
...
**Risks:** Low. Standard inquirer flow.

---

## 2.3 Template Engine -- brand.json + env vars (2h)
...
- Generated `brand-tokens.css` sets `--brand-primary`, `--brand-secondary`, `--brand-accent`, `--brand-name`, etc.
...
**Risks:** Low. Template engines are well understood.

---

## 2.4 Cloudflare Deploy Integration (2h)
...
- Sets required secrets via `wrangler secret put` (JWT_SECRET, etc.)
- If wrangler is not installed globally, auto-installs via `npx wrangler`
- Reports progress: "Deploying Worker...", "Applying migrations...", "Setting secrets..."
...
- Optionally deploys static assets to Cloudflare Pages (if configured)
...
**Risks:** Medium. Wrangler version compatibility, CF API changes. Mitigation: Pin wrangler version, run in `npx wrangler@3` to get consistent behavior.

---

## 2.5 Seed Data -- Menu Categories + Default Items (1.5h)

**Files:**
- `setup/aura-deploy/src/deploy/seed-db.ts`
- `setup/aura-deploy/templates/seed.sql.hbs` (or inline SQL builder)

**Acceptance Criteria:**
- Runs seed SQL against the newly deployed D1 database
- Inserts default menu categories (Coffee, Tea, Smoothies, Frappuccino, etc.)
...
2. Categories: Cà phê, Trà, Đá xay, Sinh tố, Nước ép, Bánh ngọt
...
**Risks:** Low. SQL seed is well understood. Risk if D1 is not yet provisioned (new account). Mitigation: Check D1 exists first, offer to create.

---

## 2.6 Output -- URLs, Credentials, Next Steps (0.5h)

**Files:**
- `setup/aura-deploy/src/output/summary.ts`

**Acceptance Criteria:**
- Prints a clean, colorful summary after successful deploy:
  ```
  ========================================
  Cafe Name:    Cafe Xinh
  Admin URL:    https://cafe-xinh.workers.dev/admin
  Public URL:   https://cafe-xinh.workers.dev
  Admin Email:  admin@cafexinh.com
  Admin Pass:   ******** (shown once)
  ========================================
  Next steps:
  1. Visit Admin URL and log in
  2. Customize menu items in admin panel
  3. Point custom domain in Cloudflare Dashboard
  4. Set up PayOS payment integration
  ========================================
  ```
- Admin password shown only once, with warning to save it
- Next steps are bilingual (Vietnamese + English)
- Output is also saved to `deployments/{domain-slug}/deployment-summary.txt`

**Implementation Steps:**
1. Write summary formatter with chalk for colors
2. Generate next-steps text from templates
...
**Acceptance Criteria:**
- Template engine tests: Given a CafeConfig object, all 4 output files are generated correctly
- Token replacement tests: All `{{VAR}}` tokens in templates are replaced
- Validator tests: Invalid domain, email, color, password are rejected; valid inputs pass
...
**Risks:** Low. Standard testing patterns.
...
| Task | Hours |
|------|-------|
| 2.1 Scaffold CLI | 1.5 |
| 2.2 Init command wizard | 2.0 |
| 2.3 Template engine | 2.0 |
| 2.4 CF deploy integration | 2.0 |
| 2.5 Seed data | 1.5 |
| 2.6 Output summary | 0.5 |
| 2.7 Integration tests | 0.5 |
| Buffer/overhead | 0.5 |
| **Total** | **10.0** |
...
setup/
  aura-deploy/
    ...
    src/
      ...
      wizard/
        questions.ts              # inquirer prompt definitions
      ...
      output/
        summary.ts                # deployment summary printer
    __tests__/
      integration.test.ts
...
- Every user-facing string is bilingual (VN + EN) where applicable
