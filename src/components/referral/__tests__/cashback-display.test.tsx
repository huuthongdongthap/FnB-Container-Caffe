...
describe('CashbackDisplay', () => {
  it('displays cashback section for per-referral rate', () => {
    ...
    expect(screen.getByText(/Cashback moi luot gioi thieu/i)).toBeInTheDocument();
  });

  it('shows total cashback earned for multiple referrals', () => {
    render(<CashbackDisplay earnedAmount={50000} totalReferrals={5} />);
    expect(screen.getByText('Tien cashback da nhan')).toBeInTheDocument();
  });

  it('shows total referral count', () => {
    render(<CashbackDisplay earnedAmount={30000} totalReferrals={3} />);
    // "Tu 3 luot gioi thieu thanh cong" text split across multiple text nodes
    expect(screen.getByText(/luot gioi thieu thanh cong/)).toBeInTheDocument();
  });
  ...
  it('shows earned history section heading', () => {
    ...
    expect(screen.getByText(/Trung binh/i)).toBeInTheDocument();
  });
});
