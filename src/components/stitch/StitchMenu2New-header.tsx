...
export function Header({ brandName }: HeaderProps) {
  ...
  return (
    <header
      className="fixed top-0 z-50 w-full border-b border-[var(--aura-text-body,#c7c6c4)]/30 bg-[var(--aura-bg-page, var(--aura-bg-surface))]/80 backdrop-blur-[8px]"
    ...
    >
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-4 md:px-16">
        ...
        >
          <a
            href="#"
            className="font-body text-base uppercase tracking-wider text-[var(--aura-text-body,#c4c6ce)] transition-colors hover:text-[var(--aura-chrome-bright,#b5c8e7)]"
          ...
          <a
            href="#"
            className="border-b-2 border-[var(--aura-chrome-light,#ffb779)] pb-1 font-body text-base uppercase tracking-wider text-[var(--aura-chrome-bright,#b5c8e7)]"
        ...
        <div className="flex items-center gap-6">
          {/* Desktop Search */}
          <div
            className="hidden items-center gap-2 border-b border-[var(--aura-chrome-bright,#E5E4E2)]/30 py-1 lg:flex"
            role="search"
            aria-label={t('stitch.menu2.searchAriaLabel')}
          >
            <Search className="h-4 w-4 text-[var(--aura-text-muted,#8e9097)]" aria-hidden="true" />
            <input
              type="text"
              placeholder={t('stitch.menu2.searchPlaceholder')}
              aria-label={t('stitch.menu2.searchAriaLabel')}
              className="w-48 border-none bg-transparent font-body text-sm font-medium text-[var(--aura-text-body,#c4c6ce)] placeholder-[var(--aura-text-muted,#8e9097)]/50 focus:outline-none focus:ring-0"
            />
          </div>
