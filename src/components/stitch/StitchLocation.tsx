import { AuraImage } from '@/components/ui/AuraImage';

export interface StitchLocationProps {
  className?: string;
}

interface ContactInfo {
  icon: 'location' | 'schedule' | 'phone';
  title: string;
  content: string;
}

interface SocialLink {
  name: string;
  icon: 'facebook' | 'instagram' | 'tiktok';
  href: string;
}

const mockData = {
  sectionLabel: 'Tim Chung Toi',
  title: 'Diem Den Moi Cua Sa Dec',
  mapImageUrl:
    'https://lh3.googleusercontent.com/aida-public/AB6AXuDw43NEHsv9qzJghOBcIJ4BwCjCAeul8BnNWu4ecdj_t-zwl1_AIqzyRgdYFgy8iCxUh-yCXsvogxH6Fom6nUwHOtsBG4mV-lirGT7yxQEortpq4QZBj2XsXF9GaOfe-gFwoAtAlFhAxF7_m_RWbZKXj8VYLjOyXaiVqH00vayfEKLWByvstdAl_oCHv5_EVYRuII8Z0590Mhz3Ip9gki0y9APtoU7BlWrnipIt0zQiIVUgdvrXVuQGPlaCwi4qtqUUCFAZEstgoNLy',
  contacts: [
    {
      icon: 'location' as const,
      title: 'Dia Chi',
      content: '39 Nguyen Tat Thanh, TP Sa Dec, Dong Thap',
    },
    {
      icon: 'schedule' as const,
      title: 'Gio Mo Cua',
      content: 'T2 - CN: 07:00 - 22:30',
    },
    {
      icon: 'phone' as const,
      title: 'Lien He',
      content: '+84 901 234 567',
    },
  ],
  socials: [
    { name: 'Facebook', icon: 'facebook' as const, href: '#' },
    { name: 'Instagram', icon: 'instagram' as const, href: '#' },
    { name: 'TikTok', icon: 'tiktok' as const, href: '#' },
  ],
};

function ContactIcon({ icon }: { icon: ContactInfo['icon'] }) {
  switch (icon) {
    case 'location':
      return (
        <svg className="w-8 h-8 text-[var(--aura-chrome-light)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
      );
    case 'schedule':
      return (
        <svg className="w-8 h-8 text-[var(--aura-chrome-light)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      );
    case 'phone':
      return (
        <svg className="w-8 h-8 text-[var(--aura-chrome-light)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
        </svg>
      );
  }
}

function SocialIcon({ icon }: { icon: SocialLink['icon'] }) {
  const cls = 'w-5 h-5 invert';
  switch (icon) {
    case 'facebook':
      return (
        <svg className={cls} fill="currentColor" viewBox="0 0 24 24">
          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
        </svg>
      );
    case 'instagram':
      return (
        <svg className={cls} fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
        </svg>
      );
    case 'tiktok':
      return (
        <svg className={cls} fill="currentColor" viewBox="0 0 24 24">
          <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
        </svg>
      );
  }
}

export default function StitchLocation({ className = '' }: Readonly<StitchLocationProps>) {
  return (
    <section
      className={
        'max-w-[1280px] mx-auto px-[24px] py-[120px] grid grid-cols-1 lg:grid-cols-2 gap-16 items-center ' +
        className
      }
    >
      {/* Left column — info */}
      <div>
        <span className="text-sm tracking-[0.1em] text-[var(--aura-chrome-light)] uppercase mb-6 block font-body font-semibold">
          {mockData.sectionLabel}
        </span>
        <h2 className="font-display text-[clamp(2.5rem,6vw,3rem)] text-[var(--aura-text-primary)] mb-8 leading-[1.2] tracking-[-0.01em] font-medium">
          {mockData.title}
        </h2>

        <div className="space-y-8">
          {mockData.contacts.map((contact) => (
            <div key={contact.title} className="flex gap-6">
              <ContactIcon icon={contact.icon} />
              <div>
                <p className="font-display text-[clamp(1.5rem,3vw,2rem)] text-[var(--aura-text-primary)] mb-1 leading-[1.3] font-medium">
                  {contact.title}
                </p>
                <p className="text-[var(--aura-text-body)] text-base leading-[1.6] font-body">
                  {contact.content}
                </p>
              </div>
            </div>
          ))}
        </div>

        {/* Social links */}
        <div className="mt-12 flex gap-4">
          {mockData.socials.map((social) => (
            <a
              key={social.name}
              href={social.href}
              className="w-12 h-12 backdrop-blur-md bg-[var(--aura-bg-page)]/60 border border-[var(--aura-chrome-light)]/20 flex items-center justify-center hover:bg-[var(--aura-chrome-light)] hover:text-black transition-all"
              aria-label={social.name}
            >
              <SocialIcon icon={social.icon} />
            </a>
          ))}
        </div>
      </div>

      {/* Right column — map image */}
      <div className="h-[500px] w-full overflow-hidden border border-[var(--aura-border-soft)]/30 grayscale hover:grayscale-0 transition-all duration-1000">
        <AuraImage
          src={mockData.mapImageUrl}
          alt="Aura Cafe location map"
          className="w-full h-full object-cover"
        />
      </div>
    </section>
  );
}
