    ...
    <>
      ...
      <main id="main-content" className="mx-auto max-w-6xl px-4 py-8">
        ...
        <section className="mb-16 text-center">
          <div className="mb-2 font-utility text-xs font-semibold uppercase tracking-[4px] text-accent">
          ...
          <p className="mx-auto mt-4 max-w-3xl text-muted">
            ...
            thép công nghiệp và bầu trời đêm. Element DNA: <code className="rounded bg-accent/10 px-2 py-0.5 font-mono text-xs text-accent">水 Thủy — Noir Lounge</code>.
          </p>
        ...
        <section className="mb-16 scroll-mt-20" id="colors">
          <div className="mb-8">
            ...
            <p className="mt-2 text-muted">
              ...
              <code className="ml-1 rounded bg-accent/10 px-2 py-0.5 font-mono text-xs text-accent">css/brand-tokens.css</code>.
            </p>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="px-6 py-3 font-utility text-xs font-semibold uppercase tracking-wider text-muted">Token</th>
                  <th className="px-6 py-3 font-utility text-xs font-semibold uppercase tracking-wider text-muted">Size</th>
                  <th className="px-6 py-3 font-utility text-xs font-semibold uppercase tracking-wider text-muted">Usage</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {[
                  ...
                  <tr key={row.token} className="hover:bg-accent/5">
                    <td className="px-6 py-3 font-mono text-xs text-accent">{row.token}</td>
                    <td className="px-6 py-3 text-foreground">{row.size}</td>
                    <td className="px-6 py-3 text-muted">{row.usage}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="mb-16 scroll-mt-20" id="materials">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {MATERIALS.map((material) => (
              <div key={material.name} className="rounded-2xl border border-border bg-card p-6">
                <h3 className="font-display text-lg font-semibold text-foreground">{material.name}</h3>
                <p className="mt-2 text-sm text-muted">{material.desc}</p>
                <p className="mt-3 text-xs text-accent">{material.spec}</p>
              </div>
            ))}
          </div>
        </section>
