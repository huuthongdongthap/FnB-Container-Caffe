            ...
            {FILTERS.map((filter) => (
              <button
                ...
                className={`px-6 py-2 rounded-full font-body text-xs uppercase tracking-widest transition-all ${
                  filter === activeFilter
                    ? 'bg-white/5 backdrop-blur-[8px] border border-[var(--aura-tertiary)]/30 text-[var(--aura-tertiary)]'
                    : 'bg-white/5 backdrop-blur-[8px] border border-white/10 text-[var(--aura-chrome-dark)]/70 hover:text-[var(--aura-chrome-bright)] hover:border-[var(--aura-chrome-mid)]/20'
                }`}
              >
                {filter}
              </button>
