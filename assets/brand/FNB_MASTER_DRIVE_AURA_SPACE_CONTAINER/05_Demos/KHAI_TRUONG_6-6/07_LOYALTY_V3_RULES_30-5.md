...
| Rule | v2 cũ | v3 mới |
|---|---|---|
| Min order áp dụng cashback | 30k | 30k ✅ giữ |
| Max cashback / giao dịch | 50k | 50k ✅ giữ (campaign: 100k) |
| Min order khi dùng ví | 30k | 30k ✅ giữ |
...
| 3️⃣ Min order earn | MIN_ORDER_FOR_CASHBACK | 30.000đ |
...
- **Tặng 10.000đ cashback** cho **NGƯỜI GIỚI THIỆU** khi:
  ...
  - Friend có **đơn đầu tiên ≥ 30.000đ**
...
4. B đặt đơn đầu ≥ 30k → status `pending` → `confirmed`
...
- Min order 30k để tránh order ảo
...
//   - When referred has order ≥ 30k → mark status='confirmed', grant +10k to referrer
```
