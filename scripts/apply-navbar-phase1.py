...
def inject_shared_nav(content, page_key):
    ...
    content = re.sub(
        r'<div\s+id=["\']shared-navbar["\'][^>]*>.*?</div>', "", content, flags=re.DOTALL
    )
    # Remove existing <div id="shared-footer">...</div>
    content = re.sub(
        r'<div\s+id=["\']shared-footer["\'][^>]*>.*?</div>', "", content, flags=re.DOTALL
    )
    ...
    content = re.sub(
        r'<script\s+type=["\']module["\']>\s*import[^{]*\{\s*initNavbar.*?initFooter\(\);\s*</script>',
        "",
        content,
        flags=re.DOTALL,
    )
